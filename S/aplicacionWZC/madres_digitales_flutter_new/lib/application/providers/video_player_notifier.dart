...
class VideoPlayerNotifier extends StateNotifier<VideoPlayerState> {
  ...
  Future<void> loadVideo(String videoUrl) async {
    ...
    state = state.copyWith(isLoading: true, error: null, currentVideoUrl: videoUrl);
    ...
    try {
      ...
      state = state.copyWith(isLoading: false, chewieController: chewieController, videoPlayerController: videoController);
    } catch (e) {
  ...
  Future<void> _downloadVideoInBackground(String videoUrl) async {
    try {
      ...
      await _cacheService.cacheVideo(videoUrl, onProgress: (progress) {
        state = state.copyWith(downloadProgress: progress * 100);
      });
      state = state.copyWith(isDownloading: false, isCached: true);
    } catch (e) {
      state = state.copyWith(isDownloading: false, error: 'Error al descargar video: $e');
    }
  }
...
final videoPlayerProvider = StateNotifierProvider.family<VideoPlayerNotifier, VideoPlayerState, String>((ref, videoUrl) {
