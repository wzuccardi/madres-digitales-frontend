...
import 'package:madres_digitales_flutter_new/presentation/pages/alertas/alertas_screen.dart';
...
class _AlertasDashboardScreenState extends ConsumerState<AlertasDashboardScreen> {
  ...
  Future<void> _cargarEstadisticas() async {
    ...
    try {
      ...
      final todasAlertas = await alertaService.getActiveAlertas();
      
      // Calcular estadísticas por prioridad
      _estadisticasPrioridad = {
        'critica': todasAlertas.where((a) => a.nivelPrioridad == AlertaNivel.critica).length,
        'alta': todasAlertas.where((a) => a.nivelPrioridad == AlertaNivel.alta).length,
        'media': todasAlertas.where((a) => a.nivelPrioridad == AlertaNivel.media).length,
        'baja': todasAlertas.where((a) => a.nivelPrioridad == AlertaNivel.baja).length,
      };
      ...
      for (final tipo in tiposUnicos) {
        _estadisticasTipo[tipo.toString().split('.').last] = todasAlertas.where((a) => a.tipoAlerta == tipo).length;
      }
      
      // Alertas críticas pendientes
      _alertasCriticasPendientes = todasAlertas
          .where((a) => a.nivelPrioridad == AlertaNivel.critica && !a.esResuelta)
          .take(5)
          .toList();
      ...
      _resumenGeneral = {
        ...
        'pendientes': todasAlertas.where((a) => a.esPendiente).length,
        'resueltas': todasAlertas.where((a) => a.esResuelta).length,
  ...
  List<Map<String, dynamic>> _calcularTendenciaSemanal(List<Alerta> alertas) {
    ...
    for (int i = 6; i >= 0; i--) {
      ...
      tendencia.add({
        ...
        'criticas': alertasDelDia.where((a) => a.nivelPrioridad == AlertaNivel.critica).length,
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(alerta.titulo, style: const TextStyle(fontWeight: FontWeight.bold)),
                Text(
                  alerta.descripcion,
