...
import 'package:madres_digitales_flutter_new/presentation/providers/service_providers.dart';
...
class _GestanteFormMejoradoScreenState extends ConsumerState<GestanteFormMejoradoScreen> {
  ...
  Future<void> _loadMunicipios() async {
    try {
      ...
      setState(() {
        _municipiosList = municipios.map((m) => <String, dynamic>{
          'id': m.id,
          'nombre': m.nombre,
        }).toList();
  ...
  void _initializeForm() {
    if (widget.gestante != null) {
      ...
      if (g['coordenadas'] != null) {
        try {
          final coordenadas = g['coordenadas'];
          if (coordenadas is Map) {
            _latitud = coordenadas['latitud']?.toDouble();
            _longitud = coordenadas['longitud']?.toDouble();
          } else if (coordenadas is String) {
            // Si es un string con formato "lat,lng"
            final parts = coordenadas.split(',');
            if (parts.length == 2) {
              _latitud = double.tryParse(parts[0].trim());
              _longitud = double.tryParse(parts[1].trim());
            }
          }
        } catch (e) {
          AppLogger.error('GestanteForm: Error al cargar coordenadas', error: e);
        }
      }
    }
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Número de embarazo
          TextFormField(
            controller: TextEditingController(text: _numeroEmbarazo.toString()),
            decoration: const InputDecoration(
  ...
  Future<void> _guardarGestante() async {
    ...
    try {
      ...
      // Si se requiere pre-selección por sesión, usar provider de sesión en el futuro
      ...
      if (!online) {
        ...
        gestanteData['id'] = 'offline_${DateTime.now().millisecondsSinceEpoch}';
        await sync.markForSync('gestantes', gestanteData);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Sin conexión: guardado local y se sincronizará cuando esté en línea'),
              backgroundColor: Colors.orange,
            ),
          );
        ...
        // Crear el objeto Gestante
        final gestante = Gestante(
          id: widget.gestante?['id'] ?? '',
          nombre: _nombreController.text,
          apellido: '', // El formulario no lo pide, se puede agregar
          documento: _documentoController.text,
          telefono: _telefonoController.text,
          direccion: _direccionController.text,
          email: '', // El formulario no lo pide
          ...
          madrinaId: null, // Se asigna en el backend
          ipsId: null, // Se asigna en el backend
          medicoId: null, // Se asigna en el backend
          fechaUltimoControl: null,
          semanasGestacion: null,
          estado: 'activo',
          municipioId: _selectedMunicipioId,
          createdAt: widget.gestante != null ? DateTime.parse(widget.gestante!['createdAt']) : DateTime.now(),
          updatedAt: DateTime.now(),
          tipoDocumento: _tipoDocumento,
          fechaUltimaMestruacion: _fechaUltimaMenstruacion,
          barrio: '',
          coordenadas: _latitud != null && _longitud != null 
              ? '$_latitud,$_longitud'
              : null,
          numeroEmbarazo: _numeroEmbarazo,
        );

        // Convertir a formato API
        final gestanteData = GestanteConverter.gestanteToApi(gestante);
        
        // Llamada online al backend
        final apiService = ref.read(apiServiceProvider);
        if (widget.gestante == null) {
          await apiService.post('/api/gestantes', data: gestanteData);
        } else {
          await apiService.put('/api/gestantes/${widget.gestante!['id']}', data: gestanteData);
        }
      }
  ...
  Future<void> _captureCoordinates() async {
    ...
    try {
      ...
      AppLogger.error('GestanteForm: Error capturando coordenadas', error: e);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
