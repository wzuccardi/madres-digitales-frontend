...
// Corrección: Importar AlertaService (incluye el modelo Alerta)
...
class _CentroNotificacionesScreenState extends ConsumerState<CentroNotificacionesScreen>
  ...
  Future<void> _cargarAlertas() async {
    ...
    try {
      ...
      final criticas = alertas.where((a) =>
        a.nivelPrioridad == AlertaNivel.critica && !a.esResuelta).toList();
      final altas = alertas.where((a) =>
        a.nivelPrioridad == AlertaNivel.alta && !a.esResuelta).toList();
      final medias = alertas.where((a) =>
        (a.nivelPrioridad == AlertaNivel.media || a.nivelPrioridad == AlertaNivel.alta) && !a.esResuelta).toList();

      setState(() {
        _alertas = alertas.where((a) => !a.esResuelta).toList();
  ...
  Widget _buildAlertaCard(Alerta alerta) {  // Corrección: Usar Alerta
    final isCritica = alerta.nivelPrioridad == AlertaNivel.critica;
    final isAlta = alerta.nivelPrioridad == AlertaNivel.alta;
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          alerta.tipoAlerta.name.toUpperCase(),
                          style: TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                            color: iconColor,
                        ...
                        Text(
                          'Tipo: ${alerta.tipoAlerta.name.toUpperCase()}',
                          style: TextStyle(
                            fontSize: 12,
                            color: iconColor,
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Row(
          children: [
            Icon(
              alerta.nivelPrioridad == AlertaNivel.critica ? Icons.error :
              alerta.nivelPrioridad == AlertaNivel.alta ? Icons.warning :
              Icons.info,
              color: alerta.nivelPrioridad == AlertaNivel.critica ? Colors.red :
                     alerta.nivelPrioridad == AlertaNivel.alta ? Colors.orange :
                     Colors.blue,
            ),
            const SizedBox(width: 8),
            Expanded(
              child: Text(
                alerta.nivelPrioridad.name.toUpperCase(),
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              _buildDetalleRow('Tipo', alerta.tipoAlerta.name.toUpperCase()),
              _buildDetalleRow('Prioridad', alerta.nivelPrioridad.name.toUpperCase()),
              _buildDetalleRow('Mensaje', alerta.descripcion),
  ...
  Future<void> _marcarComoResuelta(Alerta alerta) async {  // Corrección: Usar Alerta
    ...
    try {
      ...
      await alertaService.marcarAlertaComoLeida(alerta.id);
      messenger.showSnackBar(
        const SnackBar(content: Text('Alerta marcada como leída')),
      );
      _cargarAlertas();
    } catch (e) {
      AppLogger.error('Error marcando alerta como leída', error: e);
      messenger.showSnackBar(
  ...
  Future<void> _sincronizarLectura() async {
    try {
      ...
      final alertasNoResueltas = _alertas.where((alerta) => !alerta.esResuelta).toList();

      if (alertasNoResueltas.isEmpty) return;

      final alertaService = ref.read(alertaServiceProvider);
      for (final a in alertasNoResueltas) {
        await alertaService.marcarAlertaComoLeida(a.id);
      }
