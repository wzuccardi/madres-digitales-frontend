...
class _IPSFormScreenState extends ConsumerState<IPSFormScreen> {
  ...
  Future<void> _loadMunicipios() async {
    try {
      ...
      setState(() {
        _municipios = municipiosData.map((m) => {
          'id': m.id,
          'nombre': m.nombre,
        }).toList();
  ...
  Future<void> _saveIPS() async {
    ...
    try {
      // datos agrupados no utilizados eliminados

      int nivelToInt(String n) {
        switch (n) {
          case 'primario':
            return 1;
          case 'secundario':
      ...
      if (widget.ips != null) {
        ...
        await _ipsService.actualizarIPS(
          id: id,
          ...
          departamento: '',
          tipo: 'ips',
          nivel: nivelToInt(_nivelAtencion),
          servicios: const [],
        );
        AppLogger.info('IPSFormScreen: IPS actualizada exitosamente');
      } else {
        await _ipsService.crearIPS(
          nombre: _nombreController.text.trim(),
          nit: _nitController.text.trim(),
          direccion: _direccionController.text.trim(),
          telefono: _telefonoController.text.trim(),
