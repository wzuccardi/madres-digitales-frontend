...
class MediaManagerWidget extends ConsumerStatefulWidget { // en MB
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        try {
          for (final file in result.files) {
            ...
            if (tempPath != null) {
              final uploadResult = await FileService.uploadFile(
                File(tempPath),
                additionalFields: {'type': _getFileType(file.extension ?? '')},
              );
              if (uploadResult.success && uploadResult.fileUrl != null) {
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text('❌ Error al subir ${file.name}: ${uploadResult.errorMessage ?? 'error desconocido'}'),
                      backgroundColor: Colors.red,
                    ),
                  );
                }
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('✅ ${_files.length} archivo(s) subido(s)'),
