...
class MedicoFormScreen extends ConsumerStatefulWidget {
  ...
  const MedicoFormScreen({super.key, this.medico, this.medicoId});
...
class _MedicoFormScreenState extends ConsumerState<MedicoFormScreen> {
  ...
  late MunicipioService _municipioService;
  ...
  void initState() {
    ...
    if (widget.medico != null) {
      _loadMedicoData();
    } else if (widget.medicoId != null && widget.medicoId!.isNotEmpty) {
      _loadMedicoById(widget.medicoId!);
    }
  }
  ...
  Future<void> _loadMedicoById(String id) async {
    try {
      final m = await _medicoService.getMedicoById(id);
      if (m != null) {
        _nombreController.text = m['nombre']?.toString() ?? '';
        _documentoController.text = m['documento']?.toString() ?? '';
        _telefonoController.text = m['telefono']?.toString() ?? '';
        _emailController.text = m['email']?.toString() ?? '';
        _especialidadController.text = m['especialidad']?.toString() ?? '';
        _registroMedicoController.text = m['registro_medico']?.toString() ?? '';
        _activo = (m['activo'] is bool) ? m['activo'] as bool : true;
        _ipsId = m['ips_id']?.toString();
        _municipioId = m['municipio_id']?.toString();
      }
    } catch (e) {
      AppLogger.error('Error cargando médico por id', error: e);
    }
  }
  ...
  Future<void> _loadData() async {
    try {
      final futures = await Future.wait([
        ...
        _municipioService.getMunicipios(activo: true, limit: 200),
      ]);
      ...
      if (mounted && !_isDisposed) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted && !_isDisposed) {
            setState(() {
              final ipsEntities = futures[0] as List;
              _ipsList = ipsEntities
                  .map((e) => {
                        'id': (e as dynamic).id.toString(),
                        'nombre': (e as dynamic).nombre.toString(),
                      })
                  .toList();
              _municipios = (futures[1] as List)
                  .map((m) => {
                        'id': (m.id).toString(),
                        'nombre': m.nombre,
                      })
  ...
  Future<void> _saveMedico() async {
    ...
    try {
      ...
      if (widget.medico != null || (widget.medicoId != null && widget.medicoId!.isNotEmpty)) {
        // Actualizar médico existente
        final id = widget.medico != null ? widget.medico!['id'] : widget.medicoId!;
        await _medicoService.updateMedico(id, medicoData);
        if (mounted && !_isDisposed) {
