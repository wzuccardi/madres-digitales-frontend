...
class OfflineErrorService {
  ...
  Timer? _syncTimer;
  // NUEVO: Timer para verificación de conectividad periódica
  ...
  Future<void> captureError({
    ...
    try {
      ...
      AppLogger.debug('OfflineErrorService: Error capturado');
      ...
      AppLogger.error('Error capturando error offline', error: e);
    }
  }
  ...
  Future<bool> retryError(String errorId) async {
    try {
      ...
      if (!error.canRetry) {
        AppLogger.warn('OfflineErrorService: Error no puede ser reintentado');
        return false;
      }
      
      // Verificar si ya hay un reintento activo
      if (_activeRetries.containsKey(errorId)) {
        AppLogger.debug('OfflineErrorService: Reintento ya en progreso');
        
        try {
          return await _activeRetries[errorId]!.future;
        } catch (e) {
          AppLogger.error('Error esperando reintento activo', error: e);
          return false;
        }
      }
      ...
      AppLogger.error('Error reintentando error offline', error: e);
      return false;
    }
  }
  ...
  Future<int> retryAllErrors() async {
    try {
      AppLogger.debug('OfflineErrorService: Reintentando todos los errores pendientes');
      
      final pendingErrors = _errorQueue.where((error) => error.canRetry).toList();
      
      if (pendingErrors.isEmpty) {
        AppLogger.debug('OfflineErrorService: No hay errores pendientes para reintentar');
        return 0;
      }
      ...
      AppLogger.debug('OfflineErrorService: Reintentos completados');
      ...
      AppLogger.error('Error reintentando todos los errores', error: e);
      return 0;
    }
  }
  ...
  Future<bool> resolveError(String errorId) async {
    try {
      ...
      if (errorIndex == -1) {
        AppLogger.warn('OfflineErrorService: Error no encontrado');
        return false;
      }
      ...
      AppLogger.debug('OfflineErrorService: Error marcado como resuelto');
      ...
      AppLogger.error('Error resolviendo error offline', error: e);
      return false;
    }
  }
  ...
  Future<bool> removeError(String errorId) async {
    try {
      ...
      if (_errorQueue.length < initialLength) {
        ...
        AppLogger.debug('OfflineErrorService: Error eliminado de la cola');
        
        return true;
      }
      
      AppLogger.warn('OfflineErrorService: Error no encontrado para eliminar');
      
      return false;
    } catch (e) {
      AppLogger.error('Error eliminando error offline', error: e);
      return false;
    }
  }
  ...
  Future<void> clearAllErrors() async {
    try {
      AppLogger.debug('OfflineErrorService: Limpiando todos los errores');
      ...
      AppLogger.error('Error limpiando todos los errores', error: e);
    }
  }
  ...
  Future<List<OfflineError>> getAllErrors() async {
    try {
      ...
      AppLogger.error('Error obteniendo todos los errores', error: e);
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> getErrorStats() async {
    try {
      ...
      AppLogger.error('Error obteniendo estadísticas de errores', error: e);
      return {};
    }
  }
  ...
  void setRetryStrategy(RetryStrategy strategy) {
    ...
    AppLogger.debug('OfflineErrorService: Estrategia de reintento actualizada');
  }
  ...
  void setConnectivityStatus(bool isOnline) {
    if (_isOnline != isOnline) {
      ...
      AppLogger.debug('OfflineErrorService: Estado de conectividad actualizado');
  ...
  Future<void> _addErrorToQueue(OfflineError error) async {
    ...
    if (existingErrorIndex != -1) {
      ...
      AppLogger.debug('OfflineErrorService: Error existente actualizado');
    } else {
      ...
      AppLogger.debug('OfflineErrorService: Error agregado a la cola');
    }
  ...
  Future<bool> _retryError(OfflineError error) async {
    ...
    try {
      AppLogger.debug('OfflineErrorService: Reintentando error');
      ...
      if (success) {
        ...
        AppLogger.debug('OfflineErrorService: Error resuelto exitosamente');
      } else {
        // Verificar si se puede volver a reintentar
        if (updatedError.canRetry) {
          AppLogger.debug('OfflineErrorService: Reintento fallido, se programará próximo reintento');
          ...
          AppLogger.warn('OfflineErrorService: Error marcado como fallido permanentemente');
      ...
      AppLogger.error('Error en reintento de error', error: e);
  ...
  Future<bool> _retryRequest({
    ...
    if (_apiService == null) {
      AppLogger.warn('OfflineErrorService: API service no disponible para reintento');
      return false;
    }
    ...
    try {
      ...
      switch (method.toUpperCase()) {
        case 'GET':
          final resp = await _apiService!.get<dynamic>(endpoint);
          success = resp.success;
          break;
        case 'POST':
          final resp = await _apiService!.post<dynamic>(endpoint, data: requestData);
          success = resp.success;
          break;
        case 'PUT':
          final resp = await _apiService!.put<dynamic>(endpoint, data: requestData);
          success = resp.success;
          break;
        case 'DELETE':
          final resp = await _apiService!.delete<dynamic>(endpoint);
          success = resp.success;
          break;
        default:
          AppLogger.warn('OfflineErrorService: Método HTTP no soportado');
          return false;
      }
      
      return success;
    } catch (e) {
      AppLogger.error('Error reintentando solicitud HTTP', error: e);
      return false;
    }
  }
  ...
  Future<void> _retryPendingErrors() async {
    try {
      ...
      AppLogger.debug('OfflineErrorService: Reintentando errores pendientes');
      ...
      AppLogger.error('Error reintentando errores pendientes', error: e);
    }
  }
  ...
  DateTime _calculateNextRetryTime(DateTime now, int retryCount, String errorType) {
    Duration delay = const Duration(seconds: 1); // Valor inicial por defecto
  ...
  Future<void> _saveErrorQueue() async {
    try {
      ...
      AppLogger.error('Error guardando cola de errores', error: e);
    }
  }
  ...
  Future<void> _loadErrorQueue() async {
    try {
      ...
      if (errorQueueJson != null) {
        ...
        AppLogger.debug('OfflineErrorService: Cola de errores cargada');
      }
    } catch (e) {
      AppLogger.error('Error cargando cola de errores', error: e);
    }
  }
  ...
  Future<void> _checkConnectivity() async {
    try {
      bool nowOnline = _isOnline;
      if (_apiService != null) {
        final resp = await _apiService!.get<Map<String, dynamic>>('/auth/ping');
        nowOnline = resp.success;
      } else {
        nowOnline = true;
      }
