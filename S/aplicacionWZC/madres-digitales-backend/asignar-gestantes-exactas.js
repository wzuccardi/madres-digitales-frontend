...
async function main() {
  ...
  try {
    console.log('🔧 Asignando gestantes existentes a la madrina Crepu...');
    
    // IDs exactos de las gestantes según los datos proporcionados por el usuario
    const gestantesIds = [
    ...
    // Para cada gestante, verificar y asignar
    for (const gestanteId of gestantesIds) {
      console.log(`\n🔍 Procesando gestante ID: ${gestanteId}`);
      ...
      if (!gestante) {
        console.log(`   ❌ Gestante no encontrada`);
        continue;
      }
      
      console.log(`   📋 Encontrada: ${gestante.nombre} - Doc: ${gestante.documento || 'N/A'}`);
      console.log(`   📍 Estado actual: madrina_id: ${gestante.madrina_id || 'NULL'}, activa: ${gestante.activa}`);
      
      // Asignar a la madrina Crepu y activar
      const gestanteActualizada = await prisma.gestantes.update({
        where: { id: gestanteId },
        data: { 
      ...
      console.log(`   ✅ Asignada a Crepu: ${gestanteActualizada.nombre} (ID: ${gestanteActualizada.id})`);
    }
    ...
    // Probar el endpoint para verificar que el filtrado funciona
