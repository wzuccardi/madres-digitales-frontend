...
class ProtectedAccessWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final roleLower = (authState.user?.role ?? sessionState.user?.role ?? '').toLowerCase();

    // Acceso total para admin y super_admin
...
class RoleProtectedWidget extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userRole = (authState.user?.role ?? sessionState.user?.role)?.toLowerCase();
    final permitted = rolesPermitidos.map((r) => r.toLowerCase()).toList();
    final tieneRolPermitido = userRole != null && permitted.contains(userRole);
