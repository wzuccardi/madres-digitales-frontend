...
import 'package:madres_digitales_flutter_new/domain/entities/map_models.dart';
...
class _MapaScreenState extends State<MapaScreen> {
    ...
    try {
      ...
      if (_ubicacionActual != null) {
        final entidades = await _geoService.buscarCercanos(
          latitud: _ubicacionActual?.latitud ?? 0.0,
          longitud: _ubicacionActual?.longitud ?? 0.0,
          radio: 50,
          tipo: 'todos',
          limit: 100,
        );
      ...
      children: [
        ...
        MarkerLayer(
          markers: [
            ...
            if (_ubicacionActual != null)
              Marker(
                point: _ubicacionActual?.latLng ?? const LatLng(0.0, 0.0),
                width: 40,
                height: 40,
                child: const Icon(
                  Icons.my_location,
  ...
  void _centrarEnUbicacionActual() {
    if (_ubicacionActual != null) {
      _mapController.move(_ubicacionActual?.latLng ?? const LatLng(0.0, 0.0), 14);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
