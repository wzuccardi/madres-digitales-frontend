...
import 'package:madres_digitales_flutter_new/data/services/sos_alarm_service.dart';
...
import 'package:madres_digitales_flutter_new/data/services/geolocalizacion_service.dart';
...
class _SOSMejoradoScreenState extends ConsumerState<SOSMejoradoScreen>
  ...
  Future<void> _obtenerUbicacion() async {
    try {
      ...
      setState(() {
        _locationStatus =
            'UbicaciÃ³n: ${_currentPosition?.latitude.toStringAsFixed(6) ?? '0.000000'}, '
  ...
  void didChangeDependencies() {
    ...
    // Suscribir al stream de SOS para mostrar overlay inmediato si llega alerta
    MensajeService().sosStream.listen((data) {
      if (!mounted) return;
      setState(() {
        _isAlertActive = true;
        _alertMessage = data['mensaje']?.toString() ?? 'Alerta SOS recibida';
      });
      _iniciarEfectosVisuales();
    });
  }
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              color: _isAlertActive 
                  ? Colors.red.withValues(alpha: _flashAnimation.value * 0.3)
                  : null,
            ),
            child: SafeArea(
              child: Column(
