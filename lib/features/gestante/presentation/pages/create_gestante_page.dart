...
import 'package:madres_digitales_flutter_new/application/providers/gestante_provider.dart';
...
class _CreateGestantePageState extends ConsumerState<CreateGestantePage> {
  ...
  Future<void> _captureCoordinates() async {
    ...
    try {
      ...
      if (position != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Coordenadas capturadas: ${position.latitude.toStringAsFixed(6)}, ${position.longitude.toStringAsFixed(6)}',
              ),
      ...
      AppLogger.error('CreateGestantePage: Error capturando coordenadas', error: e);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      final gestante = Gestante(
        ...
        madrinasAsignadas: [authState.usuario!.id],
        fotoUrl: _fotoUrl,
      );

      // Crear gestante a través del provider
      final success = await ref.read(gestanteProvider.notifier).createGestante(gestante);
