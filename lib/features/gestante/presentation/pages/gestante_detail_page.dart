...
import 'package:madres_digitales_flutter_new/core/providers/madrina_session_provider.dart';
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            backgroundColor: Colors.pink,
            child: Text(
              _gestante!.nombre.substring(0, 1),
        ...
        children: [
          _buildInformacionSection(
            ...
            [
              ...
              _buildInfoRow('Email', _gestante!.email),
          ...
          _buildInformacionSection(
            ...
            [
              _buildInfoRow(
                'Nombre',
                _gestante!.contactoEmergenciaNombre,
              ),
              _buildInfoRow(
                'Teléfono',
                _gestante!.contactoEmergenciaTelefono,
              ),
            ],
          ),
        ],
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: ElevatedButton.icon(
                  onPressed: () => context.go('${AppConstants.controlsRoute}?gestanteId=${_gestante!.id}'),
                  icon: const Icon(Icons.list),
                  label: const Text('Ver controles de esta gestante'),
                ),
              ),
              ...
              if (tienePermisoCrearControl)
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    label: const Text('Nuevo control prenatal'),
                  ),
                ),
            ],
          ),
          const SizedBox(height: 24),
          Text(
            'Accede al listado de controles para gestionar el historial clínico.',
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                if (!online) {
                  final offlineSvc = ref.read(offlineErrorServiceProvider);
                  await offlineSvc.captureError(
                    type: 'delete_gestante',
                    message: 'Eliminar gestante offline',
                    endpoint: '/gestantes/${_gestante!.id}',
                    method: 'DELETE',
                    requestData: const {},
                  );
                  if (!mounted) return;
                  messenger.showSnackBar(
                    const SnackBar(
                      content: Text('Sin conexión: se eliminará al reconectar'),
                ...
                if (resp.success) {
                  ...
                  messenger.showSnackBar(
                    SnackBar(
                      content: Text('Error al eliminar: ${resp.error?.message ?? 'Error desconocido'}'),
                      backgroundColor: Colors.red,
                    ),
                  );
                }
