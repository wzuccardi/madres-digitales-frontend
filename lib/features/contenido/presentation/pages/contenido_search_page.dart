...
import 'package:madres_digitales_flutter_new/features/contenido/presentation/providers/contenido_provider.dart';
...
class _ContenidoSearchPageState extends ConsumerState<ContenidoSearchPage> {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _buildSearchResults(contenidosState),
          ),
        ],
      ),
  ...
  Widget _buildSearchResults(ContenidoSimpleState contenidosState) {
    if (!_isSearching) {
      // Mensaje inicial
      return Center(
        child: Column(
    ...
    // Cargando
    if (contenidosState.status == ContenidoSimpleStatus.loading) {
      return const LoadingWidget(message: 'Buscando contenidos...');
    }
    if (contenidosState.status == ContenidoSimpleStatus.failure) {
      return CustomErrorWidget(
        message: contenidosState.error ?? 'Error desconocido',
        onRetry: () => _performSearch(),
      );
    }

    // Sin resultados
    final resultados = contenidosState.contenidos;
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: Text(
            '${resultados.length} resultados para "$_lastSearchQuery"',
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              final contenido = resultados[index];
              return Padding(
