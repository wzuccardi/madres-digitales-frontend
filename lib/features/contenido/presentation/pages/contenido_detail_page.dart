...
class _ContenidoDetailPageState extends ConsumerState<ContenidoDetailPage>
  ...
  Widget build(BuildContext context) {
    final contenidoAsync = ref.watch(contenidoByIdProvider(widget.contenidoId));
    final contenido = widget.contenido ?? contenidoAsync.maybeWhen(
      data: (result) => result.data,
      orElse: () => null,
    );

    // Mostrar loading si está cargando
    if (_isLoading || contenidoAsync.isLoading) {
    ...
    if (_error != null || contenidoAsync.hasError) {
      return Scaffold(
        appBar: AppBar(
          title: const Text('Error'),
        ),
        body: CustomErrorWidget(
          message: _error ?? (contenidoAsync.hasError ? contenidoAsync.error.toString() : 'Error desconocido'),
          onRetry: () {
            if (widget.contenido == null) {
              _loadContenido();
            } else {
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInfoSection('Información general', [
            _buildInfoRow('Tipo', contenido.tipo.nombre),
            _buildInfoRow('Categoría', contenido.categoria.nombre),
  ...
  Widget _buildRelacionadoTab(Contenido contenido) {
    final relacionadosAsync = ref.watch(contenidosRelacionadosProvider(
      GetContenidosParams(
    ...
    if (relacionadosAsync.isLoading) {
      return const Center(child: CircularProgressIndicator());
    }
    if (relacionadosAsync.hasError) {
      return Center(
        child: Text('Error cargando contenidos relacionados: ${relacionadosAsync.error}'),
      );
    }
    final relacionados = relacionadosAsync.when(
      data: (result) => (result.data ?? const <Contenido>[])
          .where((c) => c.id != contenido.id)
          .take(5)
          .toList(),
      loading: () => const <Contenido>[],
      error: (_, __) => const <Contenido>[],
    );
      ...
      itemBuilder: (context, index) {
        ...
        return Padding(
          ...
          child: ContenidoCardEnhanced(
            ...
            onTap: () {
              final path = AppConstants.contenidoDetailRoute.replaceFirst(':id', relacionado.id);
              context.push(path);
            },
            onToggleFavorito: () {
