...
class GeolocalizacionService {
  ...
  Future<PuntoGeo?> obtenerUbicacionActual() async {
    try {
      ...
      if (!serviceEnabled) {
        _logger.warn('Servicio de ubicación deshabilitado');
        return null;
      }
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          _logger.warn('Permiso de ubicación denegado');
          return null;
        }
      }
      ...
      return PuntoGeo(position.latitude, position.longitude);
    } catch (e, stackTrace) {
  ...
  Future<RutaCalculada> calcularRuta({
    ...
    try {
      ...
      final ruta = RutaCalculada.fromJson(response.data['data'] as Map<String, dynamic>);
